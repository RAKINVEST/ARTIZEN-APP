...
/// the wizard is already on screen.
...
Future<void> _pumpRouted(WidgetTester tester, QuotesRepository quotesRepo) async {
    ...
    routes: [
      ...
      GoRoute(
        ...
        builder: (context, state) =>
            Scaffold(body: Center(child: Text('PDF ${state.pathParameters['id']}'))),
      ),
    ],
  );
  await tester.pumpWidget(
...
void main() {
  testWidgets('creating the quote confirms it with its number, PDF and list actions',
      (tester) async {
    ...
    expect(find.widgetWithText(OutlinedButton, 'Ouvrir le PDF'), findsOneWidget);
    expect(find.widgetWithText(FilledButton, 'Voir mes devis'), findsOneWidget);
  });

  testWidgets('a creation failure is recoverable with Réessayer', (tester) async {
  ...
  testWidgets('end to end: build a quote, create it, and land on the devis list',
      (tester) async {
    await _pumpRouted(tester, FakeQuotesRepository([], createResult: _quote('q1', 'DEV-2026-0007')));
    ...
    await tester.pumpAndSettle();
    expect(find.text('Mes devis'), findsOneWidget); // left the wizard for the list
