...
class WizardProgressBar extends StatelessWidget {
  const WizardProgressBar({required this.currentIndex, required this.onTap, super.key});
...
class _StepDot extends StatelessWidget {
  const _StepDot({required this.step, required this.state, required this.onTap});
  ...
  Widget build(BuildContext context) {
    final (Color circle, Color fg, Color text) = switch (state) {
      _DotState.done => (ArtizenColors.success, Colors.white, ArtizenColors.textSecondary),
      _DotState.current => (ArtizenColors.gold, ArtizenColors.onGold, ArtizenColors.nightBlue),
      _DotState.upcoming => (ArtizenColors.border, ArtizenColors.textSecondary, ArtizenColors.textSecondary),
    };
          ...
          children: [
            ...
            Text(
              ...
              style: TextStyle(
                color: text,
                fontSize: 11,
                fontWeight: state == _DotState.current ? FontWeight.w700 : FontWeight.w400,
              ),
            ),
          ],
        ),
