...
## Étapes détaillées
1. **Kit** — Créer le kit (composition) → événement `KitCreated` (contrôle : invariants ; résultat : étape validée).
...
sequenceDiagram
  actor A as Artisan
  participant E_Kit as Kit
  participant BUS as Bus d'événements
  A->>E_Kit: Créer le kit (composition)
  E_Kit-->>BUS: KitCreated
```
