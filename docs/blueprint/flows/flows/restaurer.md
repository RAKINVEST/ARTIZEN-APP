...
## Mission
Remettre un objet archivé en actif.
...
## Étapes détaillées
1. **(propriétaire)** — Repasser l'objet à Actif → événement `ObjectRestored` (contrôle : invariants ; résultat : étape validée).
...
sequenceDiagram
  ...
  A->>SYS: Repasser l'objet à Actif
  SYS-->>BUS: ObjectRestored
```
