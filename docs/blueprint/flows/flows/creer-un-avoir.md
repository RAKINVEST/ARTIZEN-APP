...
## Mission
Corriger une facture par un avoir.
...
## Étapes détaillées
1. **Billing** — Émettre l'avoir (compensation) → événement `InvoiceCancelled` (contrôle : invariants ; résultat : étape validée).
...
sequenceDiagram
  ...
  A->>E_Billing: Émettre l'avoir (compensation)
  E_Billing-->>BUS: InvoiceCancelled
```
...
Correction tracée, facture non détruite (Loi 5).
