...
## Mission
Le journal métier append-only d'un objet.

## Description
Le journal métier append-only d'un objet.

## Responsabilité
Responsabilité **unique** : Le journal métier append-only d'un objet. Aucune autre.
...
## Agrégats
Contenu dans l'agrégat **l'objet historisé**.
...
## Contraintes
Append-only ; jamais modifié ni supprimé (Loi 5) ; consultable à tout moment.
