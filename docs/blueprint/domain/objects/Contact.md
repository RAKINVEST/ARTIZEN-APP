...
## Mission
Une personne à contacter (côté Customer ou Supplier).

## Description
Une personne à contacter (côté Customer ou Supplier).

## Responsabilité
Responsabilité **unique** : Une personne à contacter (côté Customer ou Supplier). Aucune autre.
...
## Agrégats
Contenu dans l'agrégat **Customer/Supplier**.
...
## Événements publiés
- `ContactCreated` — déclencheur, consommateurs et impact : voir events/.
- `ContactUpdated` — déclencheur, consommateurs et impact : voir events/.

## Événements consommés
En principe aucun (les objets cœur publient ; les moteurs consomment). Voir events/.

## Contraintes
Rattaché à un seul parent ; aucune donnée de contact partagée hors tenant.
