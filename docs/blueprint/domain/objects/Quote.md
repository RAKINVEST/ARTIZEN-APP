...
## Mission
Le devis : traduction commerciale d'interventions.

## Description
Le devis : traduction commerciale d'interventions.

## Responsabilité
Responsabilité **unique** : Le devis : traduction commerciale d'interventions. Aucune autre.
...
## Objets liés
Mission, Customer, Intervention, Template
...
## Cycle de vie
Voir OBJECT_LIFECYCLE. Résumé : Draft → Sent → Accepted | Refused (Expired)
...
## Événements publiés
- `QuoteCreated` — déclencheur, consommateurs et impact : voir events/.
- `QuoteSent` — déclencheur, consommateurs et impact : voir events/.
- `QuoteAccepted` — déclencheur, consommateurs et impact : voir events/.
- `QuoteRefused` — déclencheur, consommateurs et impact : voir events/.
- `QuoteExpired` — déclencheur, consommateurs et impact : voir events/.

## Événements consommés
En principe aucun (les objets cœur publient ; les moteurs consomment). Voir events/.

## Contraintes
Draft seul est modifiable/supprimable ; un devis signé/accepté devient **immuable** ; montants calculés par le seul calculateur.
