...
> **Version** 1.0 — **Status** Frozen — **Owner** Document & Media — **Last Update** 2026-08-02
...
## Mission
Une photo (avant/pendant/après, défaut, mesure) — facette de Document.

## Description
Une photo (avant/pendant/après, défaut, mesure) — facette de Document.

## Responsabilité
Responsabilité **unique** : Une photo (avant/pendant/après, défaut, mesure) — facette de Document. Aucune autre.
...
## Moteur propriétaire
**Document & Media** (unique — Loi 1 ; aucun propriétaire secondaire).

## Autres moteurs autorisés
Moteurs read-side (Decision, Companion, Knowledge, Performance) — **en lecture seule, via événements** (Loi 7).

## Objets liés
Document, Mission, Intervention, Knowledge

## Agrégats
Contenu dans l'agrégat **Document**.
...
## Événements publiés
- `PhotoAdded` — déclencheur, consommateurs et impact : voir events/.
- `PhotoAnnotated` — déclencheur, consommateurs et impact : voir events/.

## Événements consommés
En principe aucun (les objets cœur publient ; les moteurs consomment). Voir events/.

## Contraintes
Est un Document spécialisé ; jamais un agrégat concurrent.
...
## Évolutions prévues
Annotations, base de connaissances visuelle.
