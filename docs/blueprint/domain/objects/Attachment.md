...
## Mission
Le rattachement d'un Document à un objet métier.

## Description
Le rattachement d'un Document à un objet métier.

## Responsabilité
Responsabilité **unique** : Le rattachement d'un Document à un objet métier. Aucune autre.
...
## Objets liés
Document, Mission, Customer, Intervention
...
## Cycle de vie
Voir OBJECT_LIFECYCLE. Résumé : Actif → Détaché
...
## Événements publiés
- `DocumentAttached` — déclencheur, consommateurs et impact : voir events/.
- `DocumentDetached` — déclencheur, consommateurs et impact : voir events/.

## Événements consommés
En principe aucun (les objets cœur publient ; les moteurs consomment). Voir events/.

## Contraintes
Lien N–M ; ne duplique jamais les octets (référence par id).
