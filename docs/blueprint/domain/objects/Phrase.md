...
## Mission
Un texte réutilisable (garantie, condition, mention, SAV…).

## Description
Un texte réutilisable (garantie, condition, mention, SAV…).

## Responsabilité
Responsabilité **unique** : Un texte réutilisable (garantie, condition, mention, SAV…). Aucune autre.
...
## Événements publiés
- `PhraseCreated` — déclencheur, consommateurs et impact : voir events/.
- `PhraseUpdated` — déclencheur, consommateurs et impact : voir events/.

## Événements consommés
En principe aucun (les objets cœur publient ; les moteurs consomment). Voir events/.

## Contraintes
Classable, renommable, réutilisable, versionnable.
...
## Évolutions prévues
Bibliothèque partagée (Community).
