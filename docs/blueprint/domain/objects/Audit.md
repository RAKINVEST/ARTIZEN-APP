...
> **Version** 1.0 — **Status** Frozen — **Owner** Journal — **Last Update** 2026-08-02
...
## Mission
La trace de sécurité et d'accès (append-only).

## Description
La trace de sécurité et d'accès (append-only).

## Responsabilité
Responsabilité **unique** : La trace de sécurité et d'accès (append-only). Aucune autre.
...
## Événements publiés
- `AuditRecorded` — déclencheur, consommateurs et impact : voir events/.

## Événements consommés
En principe aucun (les objets cœur publient ; les moteurs consomment). Voir events/.

## Contraintes
Immuable ; jamais modifié ni supprimé ; distinct de History (métier) et Event (bus).
...
## Changelog
- 1.0 (2026-08-02) — Fiche initiale (Événement / Journal).
