...
## Mission
Porter l'identité visuelle de la Company (logo, couleurs, signature).

## Description
Porter l'identité visuelle de la Company (logo, couleurs, signature).

## Responsabilité
Responsabilité **unique** : Porter l'identité visuelle de la Company (logo, couleurs, signature). Aucune autre.
...
## Cycle de vie
Voir OBJECT_LIFECYCLE. Résumé : Actif
...
## Événements publiés
- `BrandingUpdated` — déclencheur, consommateurs et impact : voir events/.

## Événements consommés
En principe aucun (les objets cœur publient ; les moteurs consomment). Voir events/.

## Contraintes
Appartient à une seule Company ; les octets d'un asset ne changent pas (nouvelle version = nouvel asset).
