...
## Mission
Un produit ou une prestation réutilisable de la bibliothèque.

## Description
Un produit ou une prestation réutilisable de la bibliothèque.

## Responsabilité
Responsabilité **unique** : Un produit ou une prestation réutilisable de la bibliothèque. Aucune autre.
...
## Objets liés
Category, Supplier, Kit, Intervention
...
## Cycle de vie
Voir OBJECT_LIFECYCLE. Résumé : Actif → Désactivé (soft-delete)

## États
Actif → Désactivé (soft-delete)
...
## Événements publiés
- `ArticleCreated` — déclencheur, consommateurs et impact : voir events/.
- `ArticleUpdated` — déclencheur, consommateurs et impact : voir events/.
- `ArticleDeactivated` — déclencheur, consommateurs et impact : voir events/.

## Événements consommés
En principe aucun (les objets cœur publient ; les moteurs consomment). Voir events/.

## Contraintes
Prix et unité toujours définis ; désactivation soft (devis passés intacts).
...
## Évolutions prévues
Variantes, disponibilité (Ecosystem).
