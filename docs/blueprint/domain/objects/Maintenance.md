...
## Mission
Un entretien planifié récurrent.

## Description
Un entretien planifié récurrent.

## Responsabilité
Responsabilité **unique** : Un entretien planifié récurrent. Aucune autre.
...
## Cycle de vie
Voir OBJECT_LIFECYCLE. Résumé : Scheduled → Due → Done → Rescheduled
...
## Événements publiés
- `MaintenanceScheduled` — déclencheur, consommateurs et impact : voir events/.
- `MaintenanceDue` — déclencheur, consommateurs et impact : voir events/.
- `MaintenanceDone` — déclencheur, consommateurs et impact : voir events/.

## Événements consommés
En principe aucun (les objets cœur publient ; les moteurs consomment). Voir events/.

## Contraintes
Rattachée à un matériel/Customer ; alerte avant échéance.
