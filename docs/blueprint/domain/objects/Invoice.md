...
## Responsabilité
Responsabilité **unique** : La facture. Aucune autre.
...
## Cycle de vie
Voir OBJECT_LIFECYCLE. Résumé : Draft → Issued → Paid | Overdue (→ Cancelled via Avoir)
...
## Événements publiés
- `InvoiceCreated` — déclencheur, consommateurs et impact : voir events/.
- `InvoiceIssued` — déclencheur, consommateurs et impact : voir events/.
- `PaymentReceived` — déclencheur, consommateurs et impact : voir events/.
- `InvoiceOverdue` — déclencheur, consommateurs et impact : voir events/.

## Événements consommés
En principe aucun (les objets cœur publient ; les moteurs consomment). Voir events/.

## Contraintes
Une facture **validée ne peut plus être modifiée** ; correction uniquement par avoir.
