...
## Mission
Une pièce jointe générique (PDF, plan, PV, notice…).

## Description
Une pièce jointe générique (PDF, plan, PV, notice…).

## Responsabilité
Responsabilité **unique** : Une pièce jointe générique (PDF, plan, PV, notice…). Aucune autre.
...
## Objets liés
Photo, Attachment, Mission, Knowledge
...
## Cycle de vie
Voir OBJECT_LIFECYCLE. Résumé : Actif → Archivé (versionnable)

## États
Actif → Archivé (versionnable)
...
## Événements publiés
- `DocumentAdded` — déclencheur, consommateurs et impact : voir events/.
- `DocumentArchived` — déclencheur, consommateurs et impact : voir events/.

## Événements consommés
En principe aucun (les objets cœur publient ; les moteurs consomment). Voir events/.

## Contraintes
Propriétaire des octets ; une nouvelle version = un nouvel objet.
