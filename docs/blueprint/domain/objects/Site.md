...
## Mission
Un lieu d'intervention rattaché à un Customer.

## Description
Un lieu d'intervention rattaché à un Customer.

## Responsabilité
Responsabilité **unique** : Un lieu d'intervention rattaché à un Customer. Aucune autre.
...
## Objets liés
Customer, Building, Mission

## Agrégats
Contenu dans l'agrégat **Customer**.
...
## Événements publiés
- `SiteCreated` — déclencheur, consommateurs et impact : voir events/.
- `SiteArchived` — déclencheur, consommateurs et impact : voir events/.

## Événements consommés
En principe aucun (les objets cœur publient ; les moteurs consomment). Voir events/.

## Contraintes
Appartient à un seul Customer.
