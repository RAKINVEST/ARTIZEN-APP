...
## Mission
Modèle de document (devis/facture) propre à la Company.

## Description
Modèle de document (devis/facture) propre à la Company.

## Responsabilité
Responsabilité **unique** : Modèle de document (devis/facture) propre à la Company. Aucune autre.
...
## Objets liés
Company, Quote, Invoice, Document
...
## Événements publiés
- `TemplateCreated` — déclencheur, consommateurs et impact : voir events/.
- `TemplateArchived` — déclencheur, consommateurs et impact : voir events/.

## Événements consommés
En principe aucun (les objets cœur publient ; les moteurs consomment). Voir events/.

## Contraintes
Un modèle actif par type de document ; l'ancien est archivé, jamais détruit.
