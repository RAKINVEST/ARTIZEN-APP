...
| ADR-019 | **Règle d'or de la restitution** : *« je ne vois pas la différence avec le mien »* — l'indistinguabilité à l'œil (original vs rendu, côte à côte) est la loi suprême du renderer, gravée dans [REPRODUCTION_SPEC](REPRODUCTION_SPEC.md) v1.0 et [BRAND.md](../../../docs/BRAND.md) | Moderniser / harmoniser / « refaire dans le même esprit » sont des **régressions** ; toute différence visible entre l'original et le rendu rompt la promesse de marque, supérieure à toute décision technique. Distingue explicitement « identité appliquée » (produit actuel) de « devis restitué » (moteur gelé) | 2026-07 | Active |
| ADR-020 | **Deux modes officiels** : *Identité appliquée* (Mode 1 — notre modèle, actuel) vs *Restitution fidèle* (Mode 2 — redessin à l'identique, gelé). Mots *« à l'identique / restitution / fidèle / identité retrouvée »* **réservés au Mode 2**. Parcours renommé : Importer votre identité → Restituer votre modèle → Créer vos futurs devis | Empêche l'ambiguïté qui ferait passer le Mode 1 pour le Mode 2 ; fonde le positionnement (« les autres personnalisent votre devis ; ARTIZEN retrouve le vôtre »). Rend la promesse honnête et le parcours logique — décision de cap indépendante de toute techno | 2026-07 | Active |
| ADR-021 | **Les optimisations fines ne commencent qu'après validation sur le Starter Corpus.** L'architecture se conçoit avec peu de documents ; les optimisations *locales* (ex. derniers écarts typographiques) attendent les 5 familles. Le budget d'erreur qui pilote (ADR post-E-008) se lit sur le **corpus**, pas sur 1–2 PDF | Éviter d'optimiser 2 PDF alors que le moteur doit en reproduire des centaines : un écart dominant sur Chapot/SJE peut être un cas particulier. Tant qu'EBP/Batappli/Word/Excel ne confirment pas qu'il est **général**, l'optimiser risque un travail à refaire. Rigueur = même discipline qu'ADR-005 (corpus réel) appliquée aux *optimisations*, pas seulement aux *heuristiques* | 2026-07 | Active |
| ADR-022 | **Le PDF source est la donnée canonique ; le `.artizen` n'est jamais une donnée métier faisant foi.** Le `.artizen` est une représentation **intermédiaire, dérivée du PDF, ré-extractible à tout moment** ; il peut être supprimé, régénéré ou changer de format sans jamais toucher aux données du client | Le format `.artizen` v1 (positions absolues) est connu insuffisant pour la génération à contenu variable (ancrages relationnels) : une évolution de format est probable. Persister le `.artizen` comme donnée client créerait une **dette de migration sur données clients**. En gardant le PDF source comme seule source de vérité, toute évolution du format est **sans risque** pour le client | 2026-07 | Active |
| ADR-023 | **Le calculateur (`quotes/calculator.py`) est l'unique source de vérité des montants ; aucune autre couche n'en calcule.** La couche comportementale du `.artizen` (`CalcRule`) *décrit* la méthode de calcul d'un document, elle ne l'**exécute jamais** dans le produit | Le champ `CalcRule` invite à recalculer les totaux ; le faire diviserait la responsabilité monétaire et pourrait produire un total divergent du calculateur (invariants produit n°1 et n°3). Grave l'invariant au rang contraignant : un seul lieu calcule, partout | 2026-07 | Active |
