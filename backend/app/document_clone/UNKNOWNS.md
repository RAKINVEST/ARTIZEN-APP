...
| U-011 | Comment récupérer **nom d'entreprise** et **TVA** quand les chiffres sont *groupés* (« 948 081 807 ») et que la 1re ligne n'est pas la raison sociale ? | Regex VAT tolérant les espaces + heuristique de nom moins naïve. Mesurer sur le corpus (distincte de U-010) | **Partielle** — volet **TVA levé** (`vat_detector` : SIREN 3-3-3 espacé désormais lu, ancré par `FR`+clé, 0 faux positif sur les tests ; 3 cas de régression). Volet **nom d'entreprise** (1re ligne ≠ raison sociale) encore **ouvert** |
...
| U-016 | Les devis **image / scan** (ex. Fenêtre Diffusion : 0 texte, tout en raster) sont un **mode de reproduction distinct** : on reproduit l'image, pas la structure. Comment les traiter (les distinguer ; faut-il de l'OCR pour la Capacité 2 « futurs devis » ?) | Classer `source_kind` (natif/hybride/image) à l'ingest ; mesurer la part de scans dans le corpus ; décider si l'OCR est nécessaire | **Ouverte (observée sur Fenêtre, E-011)** |
| U-013 | Les **polices** ne sont pas embarquées → repli au rendu. Impact plus large que prévu (analyse Sprint 2, E-006) : **(1)** Typographie basse ; **(2)** rappel texte −2 à −6 %/page car le repli n'a pas certains caractères (**`ʼ` U+02BC**) et ses métriques font **re-segmenter** les spans par `fitz` (« 278-0 ter » → « 278-0 »+« ter »). La mise en page, elle, reste fidèle (positions 99,9 %). | Étude → [TYPOGRAPHY_STRATEGY.md](TYPOGRAPHY_STRATEGY.md) ; voie C (substitut métrique, E-007) **+ mise à l'échelle horizontale à la largeur d'origine (E-012)** → typo 97,7–100 % sur les 4 natifs. Voie A (police embarquée réelle) reste un hook optionnel | **Résolue → E-007 + E-012** (P1 natif) |
...
| S6 (Starter Corpus 5 familles, E-011) | 16 | 6 |
